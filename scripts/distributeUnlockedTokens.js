...
async function distributeUnlockedTokens() {
    ...
    const tokenName = await deployments.read('EdenToken', 'name')
    const decimals = await deployments.read('EdenToken', 'decimals')
    ...
    const nonce = await deployments.read('EdenToken', 'nonces', deployer)
