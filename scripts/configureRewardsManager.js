...
async function configureRewardsManager() {
    if(FORK_URL && FORK_URL.length > 0) {
        ...
        const EdenTokenDeployment = await deployments.get("EdenToken")
        ...
        const EdenToken = new ethers.Contract(EdenTokenDeployment.address, EdenTokenDeployment.abi, admin)
        ...
        await EdenToken.connect(treasury).transfer(ADMIN_ADDRESS, INITIAL_EDEN_REWARDS_BALANCE);
        ...
        await EdenToken.approve(RewardsManager.address, INITIAL_EDEN_REWARDS_BALANCE)
        await RewardsManager.addRewardsBalance(INITIAL_EDEN_REWARDS_BALANCE)
    } else {
