...
async function stakeEDEN(staker, amount) {
    decimals = await deployments.read('EdenToken', 'decimals')
    ...
    if(receipt.status) {
        console.log(`Successfully staked EDEN`);
    } else {
        log(`Error staking EDEN. Tx:`)
        log(receipt)
    }
}

async function stakeEDENWithPermit(staker, amount) {
    const edenToken = await deployments.get("EdenToken")
    tokenName = await deployments.read('EdenToken', 'name')
    decimals = await deployments.read('EdenToken', 'decimals')
    decimalMultiplier = ethers.BigNumber.from(10).pow(decimals)
    votingPowerImplementation = await deployments.get("VotingPower");
    votingPowerPrism = await deployments.get("VotingPowerPrism");
    ...
    const domainSeparator = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
          ...
          [DOMAIN_TYPEHASH, ethers.utils.keccak256(ethers.utils.toUtf8Bytes(tokenName)), ethers.utils.keccak256(ethers.utils.toUtf8Bytes("1")), ethers.provider.network.chainId, edenToken.address]
        )
      )

    const nonce = await deployments.read('EdenToken', 'nonces', staker.address)
...
async function printStakedBalance(staker) {
    ...
    const stakedBalance = await votingPower.getEDENAmountStaked(staker.address)
...
if (require.main === module) {
    getStakerSigner()
    .then((stakerSigner) => {
        stakeEDENWithPermit(stakerSigner, STAKE_AMOUNT)
        .then(() => {
...
module.exports.stakeEDEN = stakeEDEN
module.exports.stakeEDENWithPermit = stakeEDENWithPermit
